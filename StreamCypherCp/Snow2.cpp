...
Snow2::Snow2(const String &key)
{
	SetKey(key);
}

Snow2::Snow2(byte const *keyBuf, int keyLen)
{
	SetKey(keyBuf, keyLen);
}

// key settings
bool Snow2::SetKey(const String &key)
{
	return SetKey((byte const *)~key, key.GetCount());
}

bool Snow2::SetKey(byte const *keyBuf, int keyLen)
{
	byteIndex = 0;
...
// encode string
String Snow2::Encode(String const &s)
{
	StringBuffer sb(s.GetCount());
	Encode((const byte *)~s, (byte *)~sb, s.GetCount());
	return sb;
}

// encode buffer, dest on different buffer
void Snow2::Encode(byte const *sBuf, byte *dBuf, dword bufLen)
{
...
void Snow2::Encode(byte *buf, dword bufLen)
{
	Encode(buf, buf, bufLen);
}
